...
const AddDish = () => {
  ...
  const addDishes=async (e)=>{
          e.preventDefault();
          const success= await addDish(name,category,cost,selectedMenuValue,ingredients)
          console.log(success);
          if(success)
          {
            setShowToast(true);
...
useEffect(() => {
  ...
  return (
    ...
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
      ...
      <button disabled={category.length<1||name.length<1||ingredients.length<1||selectedMenuValue==''} type="submit" className="btn btn-primary" >Add Dish</button>
      </form>
    </div>
  )
}
