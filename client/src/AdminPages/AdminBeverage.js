...
const AdminBeverage = () => {
  ...
  // }, []);
  //console.log(buses)
  const filteredData =  Array.isArray(beverages) && beverages.filter(item =>
      item.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.type?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  const handleView = (id,index) => {
    const dataitem=beverages.find(da => da._id ==id)
    //const datadriver=getDriverById(driverId);
    navigate('getbeverage', { state: { beverage:dataitem,idx:index+1} });
     
  };
  const handleEdit = (id) => {
    const dataitem=beverages.find(da => da._id ==id)
    //const datadriver=getDriverById(driverId);
    navigate('editbeverage', { state: { beverage:dataitem} });
  };
...
//const getDriverById = (id) => drivers.find(d => d._id === id);
  useEffect(() => {
        ...
        const result = await getBeverages();
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4" onClick={handleClick}>Add Beverage</button>
      <div className="d-flex justify-content-between" style={{
      margin: '20px 0px 0px 15px',
      padding: '0px'}}>
        <h3 className="ms-2">Beverages Data</h3>
