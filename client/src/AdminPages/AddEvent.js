...
const AddEvent = () => {
  ...
  const addEvents=async (e)=>{
          e.preventDefault();
          const success= await addEvent(organizer,name,description,category,venue,date2,status)
          console.log(success);
          if(success)
          {
            setShowToast(true);
...
useEffect(() => {
  ...
  return (
    ...
    <div className='mx-0' style={{display:'flex'}}>
      ...
      <button disabled={name.length<1||status.length<1||category.length<1||description.length<1||organizer==''||venue==''} type="submit" className="btn btn-primary" >Add Event</button>
      </form>
    </div>
  )
}
