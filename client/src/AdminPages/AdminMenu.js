...
const AdminMenu = () => {
    ...
    const {menus,deleteMenu,getMenus}=context;
  ...
  // }, []);
  //console.log(buses)
  const filteredData =  Array.isArray(menus) && menus.filter(item =>
      item.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.type?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  const handleView = (id,index) => {
    const dataitem=menus.find(da => da._id ==id)
    //const datadriver=getDriverById(driverId);
    navigate('getmenu', { state: { menu:dataitem,idx:index+1} });
     
  };
  const handleEdit = (id) => {
    const dataitem=menus.find(da => da._id ==id)
    //const datadriver=getDriverById(driverId);
    navigate('editmenu', { state: { menu:dataitem} });
  };
...
//const getDriverById = (id) => drivers.find(d => d._id === id);
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4" onClick={handleClick}>Add Menu</button>
      <div className="container d-flex justify-content-between">
        <h3 className="ms-2">Menus Data</h3>
