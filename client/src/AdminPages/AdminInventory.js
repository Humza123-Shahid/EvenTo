...
const AdminInventory = () => {
    ...
    const {inventories,deleteInventory,getInventories}=context;
  ...
  const handleView = (id,index) => {
    const dataitem=inventories.find(da => da._id ==id)
    //const datadriver=getDriverById(driverId);
    navigate('getinventory', { state: { inventory:dataitem,idx:index+1} });
     
  };
  const handleEdit = (id) => {
    const dataitem=inventories.find(da => da._id ==id)
    //const datadriver=getDriverById(driverId);
    navigate('editinventory', { state: { inventory:dataitem} });
  };
...
//const getDriverById = (id) => drivers.find(d => d._id === id);
  ...
  return (
   <div>
      <button className="btn btn-primary mt-3 ms-4" onClick={handleClick}>Add Inventory</button>
      <div className="d-flex justify-content-between" style={{
      margin: '20px 0px 0px 15px',
      padding: '0px'}}>
        <h3 className="ms-2">Inventories Data</h3>
