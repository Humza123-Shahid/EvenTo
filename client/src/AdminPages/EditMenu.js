...
const EditMenu = () => {
    ...
    const [pricePerPerson, setPricePerPerson] = useState(Menu.pricePerPerson);
  ...
  const editMenus=(e)=>{
         e.preventDefault();
          const success=editMenu(Menu._id,name,menuType,pricePerPerson,description)
          console.log(success)
          if(success)
          {
            console.log("abc");
  ...
  return (
    ...
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
      ...
      <button disabled={pricePerPerson.length<1|| description.length<1||name==''||menuType==''} type="submit" className="btn btn-primary">Edit Menu</button>
      </form>
    </div>
  )
}
