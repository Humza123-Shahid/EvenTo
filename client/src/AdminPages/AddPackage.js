...
const AddPackage = () => {
  ...
  const addPackages=async (e)=>{
          e.preventDefault();
          const success= await addPackage(selectedEventValue,name,packageType,description,price,selectedStatusValue)
          console.log(success);
          if(success)
          {
            setShowToast(true);
  ...
  return (
    ...
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
     ...
     <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
      ...
      <button disabled={name.length<1||packageType.length<1||price.length<1||description.length<1||selectedEventValue==''} type="submit" className="btn btn-primary" >Add Package</button>
      </form>
    </div>
  )
}
