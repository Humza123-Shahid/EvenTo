...
const EditBeverage = () => {
    ...
    const [beverageType, setBeverageType] = useState(Beverage.type);

    const [price, setPrice] = useState(Beverage.price);
    const [availableQuantity, setAvailableQuantity] = useState(Beverage.availableQuantity);
  ...
  const editBeverages=(e)=>{
         e.preventDefault();
          const success=editBeverage(Beverage._id,name,beverageType,price,availableQuantity)
          console.log(success)
          if(success)
          {
            console.log("abc");
            setShowToast(true);
            setMsg("Beverage updated successfully")
            setType("success")
            setTimeout(()=>{
              setShowToast(false)
            },1500)
  ...
  return (
    ...
    <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
     ...
     <div className="mb-3 my-3 me-3" style={{width:'100%'}}>
      ...
      <button disabled={price.length<1|| availableQuantity.length<1||name==''||beverageType==''} type="submit" className="btn btn-primary">Edit Beverage</button>
      </form>
    </div>
  )
}
